...
const genrate = () => {
  if (isNaN(item.value) || item.value < 0 || item.value > 9) {
    const randomData = Math.floor(Math.random() * text.length);
    data.innerHTML = `<p> ${text[randomData]} </p> <p style="color: red; font-size: 18px;" >Please give a valid number between 1 to 9</p>`;
  } else {
    const localText = shuffle(text);
    console.log(localText);
    const datas = localText.slice(0, item.value);
    const show = datas.map((a) => {
      return `<p> ${a} </p>`;
    });
    data.innerHTML = show.join("");
  }
};
